...
async function listDevices(speakerConfig) {
    ...
    const [MiNAClient, MIoTClient] = await Promise.all([
 ...
 *
 * 逻辑：
 * 1) 优先走 @mi-gpt/next 默认链路（MiNA.play(text)）
 * 2) 失败后按型号映射回退到 MiOT.doAction(siid, aiid, text)
 */
async function tts(text) {
    ...
    try {
        const primaryResult = await MiSpeaker.play({ text });
    ...
    const fallbackCommand = resolveFallbackCommand(model, activeSpeakerConfig || {});
    ...
    const [siid, aiid] = fallbackCommand;

    try {
        const fallbackResult = await MiService.MiOT.doAction(siid, aiid, text);
        if (fallbackResult) {
            console.log(
                `[TTS Fallback] model=${model || "unknown"}, command=[${siid}, ${aiid}]`
            );
            return true;
        }
    } catch (fallbackError) {
        if (primaryError) {
            const mergedError = new Error(
                `TTS 主链路与回退链路均失败: primary=${primaryError.message}; fallback=${fallbackError.message}`
            );
            mergedError.cause = { primaryError, fallbackError };
            throw mergedError;
        }
        throw fallbackError;
    }
