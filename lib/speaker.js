...
function getActiveTTSCommandInfo() {
    ...
    const command = resolveFallbackCommand(model, activeSpeakerConfig || {});
    return {
        model,
        candidates,
        command,
...
async function tts(text) {
    ...
    if (commandInfo.command) {
        try {
            const cmdResult = await ttsByCommand(text, commandInfo.command);
...
async function ttsByCommand(text, command) {
    ...
    const resolvedCommand =
        normalizeActionCommand(command) ||
        resolveFallbackCommand(model, activeSpeakerConfig || {});
    if (!resolvedCommand) {
    ...
    if (result) {
        logDetailed(
            `[TTS Command] model=${model || "unknown"}, command=[${siid}, ${aiid}]`
        );
    }
    return {
        ok: !!result,
...
async function ttsByDefault(text) {
    ...
    if (result) {
        logDetailed("[TTS Default] MiNA.play(text) success");
    }
    return {
        ok: !!result,
        result,
