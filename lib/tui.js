...
const { normalizeSpeakerConfigForMultiTarget } = require("./config");
...
function loadConfig() {
    ...
    if (!fs.existsSync(configPath)) {
        return normalizeSpeakerConfigForMultiTarget({
            speaker: {
                userId: "",
                password: "",
                passToken: "",
...
function saveConfig(config) {
    ...
    const normalized = normalizeSpeakerConfigForMultiTarget(config);
    fs.writeFileSync(configPath, JSON.stringify(normalized, null, 4), "utf-8");
}
...
function getSpeakerList(config) {
    const cfg = normalizeSpeakerConfigForMultiTarget(config || {});
    const speakerCfg = cfg && cfg.speaker ? cfg.speaker : {};
    return Array.isArray(speakerCfg.speakers) ? speakerCfg.speakers : [];
}

function upsertSpeakerItem(config, item) {
    const cfg = normalizeSpeakerConfigForMultiTarget(config || {});
    const did = normalizeDid(item && item.did);
    if (!did) return cfg;

    const speakerCfg = cfg.speaker;
    if (!Array.isArray(speakerCfg.speakers)) speakerCfg.speakers = [];

    const idx = speakerCfg.speakers.findIndex(
        (row) => normalizeDid(row && row.did) === did
    );
    const old = idx >= 0 ? speakerCfg.speakers[idx] : {};
    const merged = {
        did,
        name:
    ...
    if (idx >= 0) {
        speakerCfg.speakers[idx] = merged;
    } else {
        speakerCfg.speakers.push(merged);
    }
    return cfg;
}

function removeSpeakerItem(config, did) {
    const cfg = normalizeSpeakerConfigForMultiTarget(config || {});
    const targetDid = normalizeDid(did);
    if (!targetDid) return cfg;

    const speakerCfg = cfg.speaker;
    speakerCfg.speakers = getSpeakerList(cfg).filter(
        (row) => normalizeDid(row && row.did) !== targetDid
    );
    if (normalizeDid(speakerCfg.defaultDid) === targetDid) {
        speakerCfg.defaultDid = "";
    }
    if (normalizeDid(speakerCfg.did) === targetDid) {
        speakerCfg.did = "";
    }
    return cfg;
}

function setDefaultSpeakerDid(config, did) {
    const cfg = normalizeSpeakerConfigForMultiTarget(config || {});
    const targetDid = normalizeDid(did);
    if (!targetDid) return cfg;

    cfg.speaker.defaultDid = targetDid;
    // 保持旧逻辑兼容：旧代码路径仍读取 speaker.did
    cfg.speaker.did = targetDid;
    return cfg;
}

function ensureDefaultSpeakerConsistency(config) {
    const cfg = normalizeSpeakerConfigForMultiTarget(config || {});
    const list = getSpeakerList(cfg);
    const defaultDid = normalizeDid(cfg.speaker.defaultDid);

    const activeDefault = list.find(
        (row) => normalizeDid(row && row.did) === defaultDid && row.enabled !== false
    );
    if (activeDefault) {
        cfg.speaker.did = activeDefault.did;
        return cfg;
    }

    const fallback = list.find((row) => row && row.enabled !== false);
    if (fallback) {
        cfg.speaker.defaultDid = normalizeDid(fallback.did);
        cfg.speaker.did = normalizeDid(fallback.did);
        return cfg;
    }

    cfg.speaker.defaultDid = "";
    if (!cfg.speaker.did || normalizeDid(cfg.speaker.did) === defaultDid) {
        cfg.speaker.did = "";
    }
    return cfg;
}
...
function resolveDisplayedDefaultCommand(speakerConfig, resolvedHint) {
    const explicit =
        speakerConfig &&
        Array.isArray(speakerConfig.ttsFallbackCommand) &&
        speakerConfig.ttsFallbackCommand.length >= 2
            ? speakerConfig.ttsFallbackCommand
            : null;

    if (explicit) {
        return { command: explicit, autoMapped: false };
    }

    if (
        resolvedHint &&
        Array.isArray(resolvedHint.command) &&
        resolvedHint.command.length >= 2
    ) {
        return { command: resolvedHint.command, autoMapped: true };
    }

    return { command: null, autoMapped: false };
}
...
function drawStatus(config) {
    const effectiveDid = config.speaker.defaultDid || config.speaker.did;
    const speakerOk = config.speaker.userId && effectiveDid;
    ...
    print(
        ...
        `${c.gray}默认设备:${c.reset} ${c.cyan}${effectiveDid || "未设置"}${c.reset}`
    );
    print("");
}
...
async function getResolvedTTSCommandByDid(config) {
    const speakerConf = (config && config.speaker) || {};
    const currentDid = speakerConf.defaultDid || speakerConf.did;
    if (
        !speakerConf.userId ||
        !(speakerConf.passToken || speakerConf.password) ||
        !currentDid
    ) {
        return null;
    }
    ...
    try {
        ...
        if (typeof speakerModule.listDevices !== "function") return null;

        const devices = await speakerModule.listDevices(speakerConf);
        if (!Array.isArray(devices) || devices.length < 1) return null;

        const targetDid = String(currentDid || "").trim();
        const target = devices.find((item) => String(item.did || "").trim() === targetDid);
        if (!target || !target.model) return null;

        if (typeof speakerModule.resolveTTSCommandForModel !== "function") return null;
        const resolved = speakerModule.resolveTTSCommandForModel(
            target.model,
            speakerConf
        );
        return {
            ...
            matchedKey: resolved && resolved.matchedKey,
...
async function selectDidFromDeviceList(config, opts = {}) {
    const returnDevice = !!(opts && opts.returnDevice);
    const speakerModule = await ensureSpeakerModule();
    const authReady =
    ...
    try {
        ...
        return returnDevice ? target.device : target.did;
    } catch (err) {
...
async function pickSpeakerFromManagedList(config, title) {
    const list = getSpeakerList(config);
    if (!list || list.length < 1) {
        print(`\n  ${c.yellow}⚠️ 当前还没有已添加的音箱${c.reset}`);
        await ask(`  ${c.dim}按回车返回...${c.reset}`);
        return null;
    }

    const defaultDid = normalizeDid(config && config.speaker && config.speaker.defaultDid);
    const items = [
        ...list.map((item, index) => {
            const did = normalizeDid(item && item.did);
            ...
            const model =
                item && typeof item.model === "string" && item.model.trim()
                    ...
                    : "unknown";
            const tags = [];
            if (did === defaultDid) tags.push("默认");
            ...
            return {
                ...
                label:
                    `${name}` +
                    `  ${c.dim}[did:${did}]${c.reset}` +
                    `  ${c.dim}[model:${model}]${c.reset}` +
                    `  ${c.dim}[${tags.join("/")}]${c.reset}`,
            };
        }),
        { key: "0", did: "", label: "返回" },
    ];
    ...
    const picked = await selectMenu({
        ...
        render: (selectedIdx) => {
            ...
            print(
                `  ${c.dim}当前已添加 ${list.length} 台；仅启用音箱可用于 Webhook 默认/路由${c.reset}`
            );
            print(`  ${c.dim}─────────────────────────────────${c.reset}`);
            drawSelectableItems(items, selectedIdx);
            print("");
        ...
        fallbackQuestion: `  ${c.cyan}▶${c.reset} 选择音箱: `,
    });

    if (picked === "0") return null;
    return list[Number(picked) - 1] || null;
}

async function handleSpeakerListManager() {
    while (true) {
        const config = loadConfig();
        const defaultDid = normalizeDid(config && config.speaker && config.speaker.defaultDid);
        const list = getSpeakerList(config);
        const enabledCount = list.filter((row) => row && row.enabled !== false).length;
        const items = [
            { key: "1", label: "查看已添加音箱" },
            { key: "2", label: "联网获取并添加音箱" },
            { key: "3", label: "设置默认音箱" },
            { key: "4", label: "启用/禁用音箱" },
            { key: "5", label: "删除音箱" },
            { key: "0", label: "返回上级" },
        ...
        const choice = await selectMenu({
            ...
            render: (selectedIdx) => {
                ...
                drawHeader();
                drawStatus(config);
                print(`\n  ${c.bold}📦 音箱列表管理${c.reset}`);
                print(`  ${c.dim}─────────────────────────────────${c.reset}`);
                print(
                    `  ${c.gray}默认音箱:${c.reset} ${defaultDid || c.dim + "(未设置)" + c.reset}`
                );
                print(
                    `  ${c.gray}已添加:${c.reset} ${list.length}  ${c.gray}已启用:${c.reset} ${enabledCount}`
                );
                print(`  ${c.dim}─────────────────────────────────${c.reset}`);
                print("");
                drawSelectableItems(items, selectedIdx);
        ...
        switch (choice) {
            case "1": {
                ...
                drawHeader();
                drawStatus(loadConfig());
                print(`\n  ${c.bold}📋 已添加音箱${c.reset}\n`);
                const cfgNow = loadConfig();
                const rows = getSpeakerList(cfgNow);
                const curDefault = normalizeDid(cfgNow && cfgNow.speaker && cfgNow.speaker.defaultDid);
                if (!rows || rows.length < 1) {
                    print(`  ${c.yellow}⚠️ 当前没有已添加的音箱${c.reset}`);
                } else {
                    rows.forEach((row, idx) => {
                        const did = normalizeDid(row && row.did);
                        const name =
                            row && typeof row.name === "string" && row.name.trim()
                                ? row.name.trim()
                                : did;
                        const model =
                            row && typeof row.model === "string" && row.model.trim()
                                ? row.model.trim()
                                : "unknown";
                        ...
                        const tags = [
                            ...
                            .join("/");
                        print(
                            `  ${idx + 1}. ${name}  ${c.dim}[did:${did}] [model:${model}] [${tags}]${c.reset}`
                        );
                    });
                }
                break;
            ...
            case "2": {
                const device = await selectDidFromDeviceList(config, { returnDevice: true });
                if (device && device.did) {
                    let next = upsertSpeakerItem(config, {
                        did: device.did,
                        name: device.name || device.did,
                        model: device.model || "",
                        enabled: true,
                    });
                    if (!normalizeDid(next.speaker.defaultDid)) {
                        next = setDefaultSpeakerDid(next, device.did);
                    }
                    next = ensureDefaultSpeakerConsistency(next);
                    saveConfig(next);
                    print(`\n  ${c.green}✅ 已添加音箱: ${device.did}${c.reset}`);
                }
                break;
            }
            case "3": {
                const cfgNow = loadConfig();
                const picked = await pickSpeakerFromManagedList(
                    cfgNow,
                    "🎯 选择默认音箱"
                );
                if (picked && picked.did) {
                    let next = upsertSpeakerItem(cfgNow, {
                        did: picked.did,
                        name: picked.name || picked.did,
                        model: picked.model || "",
                        enabled: true,
                    });
                    next = setDefaultSpeakerDid(next, picked.did);
                    next = ensureDefaultSpeakerConsistency(next);
                    saveConfig(next);
                    print(`\n  ${c.green}✅ 默认音箱已设置为: ${picked.did}${c.reset}`);
                }
                break;
            }
            case "4": {
                ...
                if (picked && picked.did) {
                    ...
                    next = ensureDefaultSpeakerConsistency(next);
                    saveConfig(next);
                    const changed = getSpeakerList(next).find(
                        (row) => normalizeDid(row && row.did) === normalizeDid(picked.did)
                    );
                    const enabled = changed && changed.enabled !== false;
                    print(
                        `\n  ${c.green}✅ ${picked.did} 已${enabled ? "启用" : "禁用"}${c.reset}`
                    );
                }
                break;
            }
            case "5": {
                ...
                if (picked && picked.did) {
                    let next = removeSpeakerItem(cfgNow, picked.did);
                    next = ensureDefaultSpeakerConsistency(next);
                    saveConfig(next);
                    print(`\n  ${c.green}✅ 已删除音箱: ${picked.did}${c.reset}`);
                }
                break;
            }
            case "0":
        ...
        print("");
        await ask(`  ${c.dim}按回车返回音箱列表管理...${c.reset}`);
    }
}
...
async function handleAccountSetup() {
    ...
    while (true) {
        try {
            resolvedHint = await getResolvedTTSCommandByDid(loadConfig());
        } catch {
            resolvedHint = null;
        }
        ...
        const items = [
            ...
            { key: "b", label: "音箱列表管理（添加/默认/启停/删除）" },
        ...
        const choice = await selectMenu({
            ...
            render: (selectedIdx) => {
                ...
                print(
                    `  ${c.gray}默认音箱:${c.reset} ${config.speaker.defaultDid || c.dim + "(未设置)" + c.reset}`
                );
                print(
                    `  ${c.gray}已添加音箱:${c.reset} ${getSpeakerList(config).length}`
                );
                const defaultCmdInfo = resolveDisplayedDefaultCommand(
                    config.speaker,
                    resolvedHint
                );
                print(
                    `  ${c.gray}默认 ttscmd:${c.reset} ${formatCommand(defaultCmdInfo.command)}${defaultCmdInfo.autoMapped ? ` ${c.dim}(自动映射)${c.reset}` : ""}`
                );
                if (resolvedHint && Array.isArray(resolvedHint.command)) {
                    const sourceText =
                        resolvedHint.source === "model"
                            ? `model:${resolvedHint.matchedKey}`
                            : "default";
                    print(
                        `  ${c.gray}did 生效命令:${c.reset} ${formatCommand(resolvedHint.command)} ${c.dim}(${resolvedHint.model} / ${sourceText})${c.reset}`
                    );
                } else {
                    print(
                        `  ${c.gray}did 生效命令:${c.reset} ${c.dim}(未解析，需先配置账号并能读取设备列表)${c.reset}`
                    );
                }
                print(
        ...
        switch (choice) {
            ...
            case "4": {
                ...
                if (val) {
                    ...
                    next = setDefaultSpeakerDid(next, val);
                    next = ensureDefaultSpeakerConsistency(next);
                    saveConfig(next);
                    print(`  ${c.green}✅ 已保存${c.reset}`);
                }
            ...
            case "5": {
                const config = loadConfig();
                const selectedDevice = await selectDidFromDeviceList(config, {
                    returnDevice: true,
                });
                if (selectedDevice && selectedDevice.did) {
                    let next = upsertSpeakerItem(config, {
                        did: selectedDevice.did,
                        name: selectedDevice.name || selectedDevice.did,
                        model: selectedDevice.model || "",
                        enabled: true,
                    });
                    next = setDefaultSpeakerDid(next, selectedDevice.did);
                    next = ensureDefaultSpeakerConsistency(next);
                    saveConfig(next);
                    print(`\n  ${c.green}✅ 已选择设备 did: ${selectedDevice.did}${c.reset}`);
                }
                break;
            }
            case "b": {
                await handleSpeakerListManager();
                break;
            }
            case "6": {
                const config = loadConfig();
                const currentInfo = resolveDisplayedDefaultCommand(
                    config.speaker,
                    resolvedHint
                );
                const current = formatCommand(currentInfo.command);
                const raw = await ask(
                    `  ${c.cyan}▶${c.reset} 默认 ttscmd ${c.dim}[当前 ${current}${currentInfo.autoMapped ? "，自动映射" : ""}]${c.reset}: `
                );
                if (raw) {
            ...
            case "a": {
                ...
                if (did) {
                    ...
                    next = setDefaultSpeakerDid(next, did);
                    next = ensureDefaultSpeakerConsistency(next);
                ...
                const finalConfig = ensureDefaultSpeakerConsistency(config);
                saveConfig(finalConfig);
                try {
                    const speakerModule = await ensureSpeakerModule();
                    if (
                        typeof speakerModule.setTTSFallbackCommand === "function" &&
                        Array.isArray(finalConfig.speaker.ttsFallbackCommand)
                    ) {
                        speakerModule.setTTSFallbackCommand(
                            finalConfig.speaker.ttsFallbackCommand
                        );
                    }
                    if (typeof speakerModule.setTTSMode === "function") {
                        speakerModule.setTTSMode(finalConfig.speaker.ttsMode || "auto");
                    }
                    if (typeof speakerModule.setDetailedLogEnabled === "function") {
                        speakerModule.setDetailedLogEnabled(!!finalConfig.speaker.verboseLog);
                    }
                } catch {
                    // ignore runtime sync error
                }
            ...
            case "1": {
                if (webhookServer) {
                    ...
                    try {
                        ...
                        print(`  ${c.dim}POST /webhook/tts    { "text": "...", "did": "可选" }${c.reset}`);
                        print(`  ${c.dim}POST /webhook/audio  { "url": "...", "did": "可选" }${c.reset}`);
                        print(`  ${c.dim}POST /webhook/volume { "volume": 50, "did": "可选" }${c.reset}`);
                        print(`  ${c.dim}POST /webhook/command { "siid": 3, "aiid": 1, "params": [], "did": "可选" }${c.reset}`);
                    } catch (err) {
                        printError(err);
                    }
                }
...
function startWebhookServer(port, host) {
    return new Promise((resolve, reject) => {
        ...
        function resolveTargetDidForData(data) {
            const cfg = loadConfig();
            const speakerCfg = (cfg && cfg.speaker) || {};
            const envDefaultDid = normalizeDid(process.env.XIAOI_DEFAULT_DID);
            const map = new Map();

            for (const row of getSpeakerList(cfg)) {
                const did = normalizeDid(row && row.did);
                if (!did) continue;
                map.set(did, { enabled: row.enabled !== false });
            }

            const legacyDid = normalizeDid(speakerCfg.did);
            const defaultDid = normalizeDid(speakerCfg.defaultDid);
            if (envDefaultDid && !map.has(envDefaultDid)) {
                map.set(envDefaultDid, { enabled: true });
            }
            if (legacyDid && !map.has(legacyDid)) map.set(legacyDid, { enabled: true });
            if (defaultDid && !map.has(defaultDid)) map.set(defaultDid, { enabled: true });

            const pickEnabled = (did, source, strict) => {
                const targetDid = normalizeDid(did);
                if (!targetDid) return null;
                const item = map.get(targetDid);
                if (item && item.enabled) {
                    return { did: targetDid, source };
                }
                if (strict) {
                    const err = new Error(`did 不存在或未启用: ${targetDid}`);
                    err.statusCode = 400;
                    throw err;
                }
                return null;
            ...
            const requestedDid = normalizeDid(data && data.did);
            if (requestedDid) {
                return pickEnabled(requestedDid, "request", true);
            }

            return (
                pickEnabled(speakerCfg.defaultDid, "defaultDid", false) ||
                pickEnabled(envDefaultDid, "env", false) ||
                pickEnabled(speakerCfg.did, "legacyDid", false) ||
                (() => {
        ...
        const server = http.createServer(async (req, res) => {
            ...
            if (req.method === "GET" && url.pathname === "/") {
                ...
                return sendJSON(200, {
                    ...
                    speaker: {
                        defaultDid: cfg && cfg.speaker ? cfg.speaker.defaultDid || "" : "",
                        envDefaultDid: normalizeDid(process.env.XIAOI_DEFAULT_DID),
                        totalSpeakers: getSpeakerList(cfg).length,
                    },
                });
            }
                ...
                try {
                    ...
                    switch (url.pathname) {
                        case "/webhook/tts": {
                            ...
                            const target = resolveTargetDidForData(data);
                            await speaker.tts(data.text, { did: target.did });
                            return sendJSON(200, {
                                success: true,
                                action: "tts",
                                text: data.text,
                        ...
                        case "/webhook/audio": {
                            ...
                            const target = resolveTargetDidForData(data);
                            await speaker.playAudio(data.url, { did: target.did });
                            return sendJSON(200, {
                                success: true,
                                action: "audio",
                                url: data.url,
                        ...
                        case "/webhook/volume": {
                            ...
                            if (!Number.isFinite(volume) || volume < 0 || volume > 100) {
                                return sendJSON(400, { error: "volume 字段必须为 0-100 的数字" });
                            }
                            const target = resolveTargetDidForData(data);
                            await speaker.setVolume(Math.round(volume), { did: target.did });
                            return sendJSON(200, {
                                success: true,
                                action: "volume",
                        ...
                        case "/webhook/command": {
                            const siid = Number(data && data.siid);
                            const aiid = Number(data && data.aiid);
                            const params = data && Array.isArray(data.params) ? data.params : [];
                            if (!Number.isFinite(siid) || !Number.isFinite(aiid)) {
                                return sendJSON(400, { error: "siid/aiid 必须为数字" });
                            }
                            const target = resolveTargetDidForData(data);
                            await speaker.doAction(siid, aiid, params, { did: target.did });
                            return sendJSON(200, {
                                success: true,
                                action: "command",
                                siid,
                    ...
                    const statusCode =
                        err && Number.isFinite(Number(err.statusCode))
...
async function handleTest() {
    const config = loadConfig();
    const targetDid = config.speaker.defaultDid || config.speaker.did;
    ...
    // 检查配置
    if (!config.speaker.userId || !targetDid) {
    ...
    print(`  ${c.gray}设备名称:${c.reset}  ${targetDid}`);
